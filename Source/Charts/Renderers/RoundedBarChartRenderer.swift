...
open class RoundedBarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        for j in stride(from: 0, to: buffer.rects.count, by: 1)
        {
            ...
            if !dataSet.isStacked || (j % 2 == 0 && dataSet.isStacked) {
                barRect.size.height += 30
            } else {
                barRect.size.height += 0.5
            }

            if j % 2 == 0 && dataSet.isStacked {
                context.fill(barRect)
            } else {
                context.saveGState()
                            ...
                            for k in 0 ..< transformed.count
                            {
                                ...
                                if dataSet.isDrawValuesEnabled
                                {
                                    drawValue(
                                        ...
                                        font: valueFont,
                                        align: .center,
                                        color: dataSet.valueTextColorAt(k % 2 == 0 ? 1 : 0))
                                }
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        for high in indices
        {
            ...
            if let e = set.entryForXValue(high.x, closestToY: high.y) as? BarChartDataEntry
            {
                ...
                prepareBarHighlight(x: e.x, y1: y1, y2: y2,
